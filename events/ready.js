...
module.exports = async (client) => {
  ...
  setInterval(() => {
    let randomUser = client.users.cache
      .filter((a) => !a.bot)
      .filter((a) => a.avatarURL({ dynamic: true, size: 1024 }))
      .random();
    ...
    axios
      ...
      .then((res) => {
        if (res.data.banner) {
          if (!client.channels.cache.get(bannerid)) return;
              ...
              ?.send({
                embeds: [
                  new MessageEmbed()
                    ...
                    .setImage(
                      `https://cdn.discordapp.com/banners/${randomUser.id}/${res.data.banner}.gif?size=2048`
                    ),
                ],
              });
          } else {
              ...
              ?.send({
                embeds: [
                  new MessageEmbed()
                    ...
                    .setFooter({ text: randomUser.id })
                    .setImage(
                      `https://cdn.discordapp.com/banners/${randomUser.id}/${res.data.banner}.png?size=2048`
                    ),
                ],
              });
          }
